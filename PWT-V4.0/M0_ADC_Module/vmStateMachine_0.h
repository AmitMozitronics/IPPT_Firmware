...
unsigned int8 StateFunc_SM0_State2(int8 i8StateMachine, int8 i8State)
{
    ...
    g_fIntermediateDataSend = IBU_INTERMEDIATE_DATA_SEND_OFF; // Dose not matter but Still for safety ...
    
    // Step 3: Stop Main Split Average Calculations and IBU Intermediate Data Sending
...
//    }
    ...
    if (IBU_RESET_CALIB_ON == g_fResetCalibStatus)
    {
        g_ui8ADCMeanCalState  = ADC_MEAN_CALCULATION_START;
    ...
    else
    {
        ...
        if(MULTI_VARIETY_PRODUCTION == g_fCurrProdType)
        {
            // Choose current variety.
            if(VARIETY_1 == g_ui8CurrentVariety)
            {
                g_ui16Var1CurrentBoxCount++;
            }
            else if(VARIETY_2 == g_ui8CurrentVariety)
            {
                g_ui16Var2CurrentBoxCount++;
            }
            else if(VARIETY_3 == g_ui8CurrentVariety)
            {
                g_ui16Var3CurrentBoxCount++;
            }
            else if(VARIETY_4 == g_ui8CurrentVariety)
            {
                g_ui16Var4CurrentBoxCount++;
            }
            else
            {
                /* Do Nothing */
        ...
        else if(SINGLE_VARIETY_PRODUCTION == g_fCurrProdType)
        {
...
unsigned int8 StateFunc_SM0_State8(int8 i8StateMachine, int8 i8State)
{
    ...
    g_fBoxLeftDelayState = BOX_LEFT_DELAY_START;
    
    // Update flag status.
    g_fExitTimeDataSendingStatus = LOCK_DATA_SENDING;
...
// State 8: Small Box Touches Exit Sensor
unsigned int8 StateFunc_SM0_State9(int8 i8StateMachine, int8 i8State)
{
    // First set the DOs
    StateFunc_GenericState(i8StateMachine, i8State);
    
    // Update Global Software DI
    g_unDIState.m_stAllDIBits.m_ui1SWDICn03 = 1;
    
    return 0;
}

// State 8: Small Box Touches Exit Sensor
unsigned int8 StateFunc_SM0_State10(int8 i8StateMachine, int8 i8State)
{
    // First set the DOs
    StateFunc_GenericState(i8StateMachine, i8State);
...
void Initialize_StateMachine_0()
{
    ...
    g_stArrStateMachines[STATE_MACHINE_0].m_stArrStateLookupTable[STATE_08].m_i8ArrExpNextState[0] = STATE_09;
    ...
    g_stArrStateMachines[STATE_MACHINE_0].m_stArrStateLookupTable[STATE_09].pFunStateFunction = StateFunc_SM0_State9;
    
//    // SM0: State 10: Box Left Exit Sensor(HOME)
//    g_stArrStateMachines[STATE_MACHINE_0].m_stArrStateLookupTable[STATE_10].m_ui32DIState = STATE_SM0_HWDI_10;
//    g_stArrStateMachines[STATE_MACHINE_0].m_stArrStateLookupTable[STATE_10].m_ui16DOState = STATE_SM0_HWDO_10;
...
//    g_stArrStateMachines[STATE_MACHINE_0].m_stArrStateLookupTable[STATE_10].pFunStateFunction = StateFunc_SM0_State10;
}    
